...
export async function generateMetadata({ params }) {
  ...
  return {
    title: post.title || post.seoOverride?.title,
    description: post.description || post.seoOverride?.description,
    openGraph: {
      images: [
        {
          url: post.coverImage?.url || post.seoOverride?.image?.url,
          width: post.coverImage?.width || post.seoOverride?.image?.width,
          height: post.coverImage?.height || post.seoOverride?.image?.height
        }
      ]
    }
  }
      ...
      >
        ...
        <div className="prose lg:pb-0 lg:row-span-2">
          <RichText
            ...
            renderers={{
              code: ({ children }) => (
                <code className="text-sm font-normal font-mono bg-gray-100 rounded-md p-1">
                  {children}
                </code>
              )
            }}
